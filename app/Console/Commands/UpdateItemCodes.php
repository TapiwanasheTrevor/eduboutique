...
{
    protected $signature = 'products:update-item-codes {--dry-run : Show what would be updated without making changes}';
    protected $description = 'Update product item codes from prices.json file';
    ...
    public function handle(): int
    {
        ...
        foreach ($products as $product) {
            ...
            $itemCode = $this->findItemCode($product, $priceIndex, $prices);
            ...
            if ($itemCode !== null) {
                ...
                if ($product->item_code != $itemCode) {
                    ...
                    if ($isDryRun && $updated <= 20) {
                        ...
                        $this->line("    Item Code: <info>{$itemCode}</info>");
                    }
                }
            } else {
                $notMatched++;
        ...
        if ($notMatched > 0 && $this->output->isVerbose()) {
            ...
            $unmatchedProducts = $products->filter(function ($p) use ($priceIndex, $prices) {
                return $this->findItemCode($p, $priceIndex, $prices) === null;
            })->take(20);
    ...
    private function buildPriceIndex(array $prices): array
    {
        ...
        foreach ($prices as $item) {
            ...
            $index[$normalized] = $item['code'];
            $index[$title] = $item['code'];
    ...
    private function findItemCode(Product $product, array $priceIndex, array $prices): ?string
    {
        ...
        if (isset($priceIndex[$normalized])) {
            return $priceIndex[$normalized];
        }

        if (isset($priceIndex[$title])) {
            return $priceIndex[$title];
        }

        // Fuzzy match - look for significant word overlap (high threshold)
        foreach ($prices as $item) {
            $priceTitle = strtolower(trim($item['title']));
            $priceNormalized = $this->normalizeTitle($priceTitle);

            if ($this->fuzzyMatch($normalized, $priceNormalized)) {
                return $item['code'];
            }
        }
    ...
    private function fuzzyMatch(string $dbTitle, string $priceTitle): bool
    {
        ...
        if (count($dbWords) < 3 || count($priceWords) < 3) {
            return false;
        }
        ...
        foreach ($dbWords as $word) {
            foreach ($priceWords as $pWord) {
                if ($word === $pWord || (strlen($word) > 4 && levenshtein($word, $pWord) <= 1)) {
                    $matches++;
                    break;
                }
            }
        ...
        // Require at least 75% word match (stricter)
